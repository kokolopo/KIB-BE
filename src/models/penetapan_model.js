...
const penetapanModel = {
  ...
  fetchTanahByDepartemen: (idDepartemen, perPage = 10, page = 1) => {
    ...
    let query = `
          SELECT aset.penetapan.*,ROW_NUMBER() OVER (ORDER BY aset.penetapan.id) AS nomor, TO_CHAR(tgl_perolehan, 'DD-MM-YYYY') AS tgl_perolehan_formated, aset.kategoris.nama
          FROM aset.penetapan
          JOIN aset.kategoris ON aset.penetapan.kategori_id = aset.kategoris.id
