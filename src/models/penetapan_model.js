...
const penetapanModel = {
  ...
  fetchAsetRusakBerat: (idDepartemen, perPage = 10, page = 1, tahun = 2024) => {
    const offset = (page - 1) * perPage;
  ...
  fetchAsetTGR: (idDepartemen, perPage = 10, page = 1, tahun = 2024) => {
    const offset = (page - 1) * perPage;
  ...
  countTotalPage: (
    ...
    WHERE 
      d.kode = '${idDepartemen}' AND p.thn_nilai = ${
      tahun - 1
    }  AND p.kondisi = '${kondisi}'
    `;

    if (kategori != null) query += `AND k.kode LIKE '%${kategori}%'`;
