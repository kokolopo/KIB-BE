  ...
  insertInventarisA: (data) => {
    ...
    let query = `
        INSERT INTO aset.kib_inventaris (
          ...
          ${data.file_nm ? `${data.file_nm}` : null},
          ${data.petugas ? `${data.petugas}` : null},
          ${data.tahun ? `${data.tahun}` : null},

          // ARRAY[${data.file_nm.map((file) => `'${file}'`).join(", ")}]::VARCHAR[],
          // ARRAY[${data.petugasmap((petugas) => `'${petugas}'`).join(", ")}]::VARCHAR[],
        )  
    `;
